...
from .models import OfertaEmpleo, Profesional, Usuario
...
class OfertaEmpleoFilter(django_filters.FilterSet):
    ...
    def filter_recent(self, queryset, name, value):
        ...
        fecha_limite = timezone.now() - timedelta(days=value)
...
class ProfesionalFilter(django_filters.FilterSet):
    ...
    # Disponibilidad
    disponibilidad = django_filters.ChoiceFilter(
        method='filter_by_disponibilidad',
        choices=[
            ('inmediata', _('Inmediata')),
            ('2_semanas', _('2 semanas')),
            ('1_mes', _('1 mes')),
            ('negociable', _('Negociable')),
        ],
    ...
    def filter_by_experience(self, queryset, name, value):
        ...
        return queryset.filter(total_experience__gte=value)
    
    def filter_by_disponibilidad(self, queryset, name, value):
        """Filtrar por disponibilidad"""
        # Esto requeriría un campo adicional en el modelo Profesional
        # Por ahora retornamos el queryset sin cambios
        return queryset